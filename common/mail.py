...
def send_email(mail_host, mail_user, mail_pass, sender, receiver, content, title):
    receivers = [receiver]
...
def send_mail_by_config(conf_dict, content_dict, title):
    ...
    send_email(mail_host, mail_user, mail_pass, sender, receiver, content, title)


def main():
    send_mail_by_file("../mail.ini", {"测试": "测试"}, "测试呢")
